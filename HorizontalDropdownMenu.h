...
public:
	...
	static	void setPlayerTravelDestination(RenderWindow& window) {
		cout << "DS"<<endl;
		cout << player->targetPos.y << endl;
		player->targetPos = cityUIHoveredOver(window)->getLocation();
		cout << player->targetPos.y << endl;
	}
	static void organizeCities() {
	}
	...
	static Cities* cityUIHoveredOver(RenderWindow& window) {
		for (unsigned int i = 0; i < sizeof(cities); i++) {
			if (buttons[i].isMouseOver(window)) {
				return &cities[i];
			}
	...
	void drawTo(RenderWindow& window) override {
		...
		//clickedCityButton = setPlayerTravelDestination;
		/*	cout << Button::allButtons[i] << endl;
			cout << &buttons[i]<< endl;
			buttons[i]->printGlobalPos();
			Button::allButtons[i]->printGlobalPos();*/
		//cout << buttons.size() << endl;
		//cout << endl<<"2: " << buttons[0] << endl;
		//cout << "this: " << this;
		//cout << "clicked2: " << CityUI::buttons[0]->clicked << endl;
		//cout << "clicked3: " << test << endl;
		for (unsigned int i = 0; i < Cities::numCities; i++) {
			//buttons[i].printGlobalPos();
			//Button::allButtons[i]->printGlobalPos();
			//cout << Button::allButtons1[i].clicked << endl;
			//Button::allButtons[i].printGlobalPos();
			...
			//cout << "size: " << buttons.size() << " i: " << i << endl;

			//cout << buttons[i].getText() << endl;

			/*buttons[i]->clicked = nullptr;
			cout << buttons[i]->clicked << endl;*/

			if (buttons[i].clicked == nullptr) {
				//system("pause");
				buttons[i].clicked = setPlayerTravelDestination;
				cout<<buttons[i].clicked << endl;
				//cout << Button::allButtons2[i].clicked << endl;
