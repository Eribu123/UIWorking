		...
		while (window.isOpen()) {
			...
			while (window.pollEvent(event)) {
				if (event.type == Event::Closed) {
					...
					for (int i = 0; i < Button::allButtons.size(); i++) {
						/* note that this code below will not work if u move the button*/
						if (Button::allButtons[i].isMouseOver(window)) {
							cout << "CLICKED1" << endl;
							//Button::allButtons[i].clicked();
							/* this is very ugly as it requires us to check for each type of panel that has buttons, 
							ideally we should just check each button straight up.
							But what doesn't make sense to me is that if you try to make the Buttons::allButtons a pointer vector, 
							it won't have any of the member vars, including the clicked function pointer,
							and in its current state once u modify the CityUI::buttons[i] it doesn't change in the Button::allButtons var*/
							if (CityUI::buttons[i].clicked != nullptr) {
								cout << player.targetPos.y << endl;
								CityUI::buttons[i].clicked(window);
								cout << player.targetPos.y << endl;

							}
							if (iUI.getItemPanels()[i-sizeof(CityUI::buttons)].getInputBox()->clicked!=nullptr) {
								cout << "checking" << endl;
							}
						}
					}
